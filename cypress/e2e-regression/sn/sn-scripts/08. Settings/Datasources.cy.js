...
if (!Cypress.env('onlyRunReadOnlyTests')) {
    ...
    `);
            ...
            // Filter out 'CyPress test Datasource'
            ...
            //cy.deleteDownloadsFolder(); (Privilegde issue)
            
            // Log number of records
            // cy.getSC(
            //     GQ.Menu.Settings.Datasources.rowCountID()
            //     , { timeout: 60000 }).invoke('text')
            //     .then((rows) => {
            ...
            //         // Read the data
            //         cy.task('readXlsx', { file: 'cypress/downloads/Results.xlsx', sheet: 'Sheet0' }).then((rows) => {
