...
describe('Sprint 6', () => {
    const GQ = new GQTest();
    it('13175-Allows a controlled list of Sites for user assignment', () => {
        ...
        // Create Site
        GQ.NavBar.ElementLibrary.UserReporting.Site.createNewSite('Test')

        // Select Test 
        GQ.NavBar.ElementLibrary.UserReporting.Site.selectSiteByText('Test')

        // Modify Name
        GQ.NavBar.ElementLibrary.UserReporting.Site.modifySitetName('Test Modified')
        
        // Modify Description
        GQ.NavBar.ElementLibrary.UserReporting.Site.modifySiteDescription('Test Function')

        // Save 
        GQ.NavBar.ElementLibrary.UserReporting.Site.save()

        // Link User
        GQ.NavBar.ElementLibrary.UserReporting.Site.linkUserByText('Cypress',0)
        ...
        .then((resultObject) => {
            ...
            sampleNumber.forEach((randomIndex) => {
                ...
                if (rowData.length > 7) {
                    let extractedValue = rowData[11].trim();
                    cy.log('Extracted 10th value: "' + extractedValue + '"');
        ...
        // Delete
        GQ.NavBar.ElementLibrary.UserReporting.Site.deleteSiteByText('Test Modified')

        // Error Message
        cy.getSC(GQ.NavBar.ElementLibrary.UserReporting.Site.message).invoke('text').then((title)=>{
            cy.wrap(title).should('equal', "STOP: You are attempting to delete an Site that has currently assigned users. First un-assign this Site from all users and then repeat this delete operation.")
        })

        // Click Ok
        GQ.NavBar.ElementLibrary.UserReporting.Site.clickOKWarningMessage()

        // Unlink user
        GQ.NavBar.ElementLibrary.UserReporting.Site.unlinkUserByIndex(0)
