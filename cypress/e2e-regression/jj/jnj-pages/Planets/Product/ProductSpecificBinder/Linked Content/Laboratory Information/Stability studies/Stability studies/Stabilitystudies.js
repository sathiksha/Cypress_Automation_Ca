export class stabilitystudies {
  ...
  checkColumns() {
    cy.fixture('stabilityStudies').then(data => {
      const expectedColumnNames = data.StabilityStudies.columnNames;
  ...
  checkMultiChoicePickerColumns() {
    cy.fixture("stabilityStudies").then(data => {
      ...
      const pickerColumns = data.StabilityStudies.multiChoicePickerColumns;  // FIX ✔
  ...
  checkFormsandFieldsAvailablity() {
    cy.fixture("stabilityStudies").then(data => {
      ...
      const sections = data.StabilityStudies.accordionSections;
  ...
  checkRelatedObjects() {
    cy.fixture("stabilityStudies").then(data => {
      const relatedObjectsOption = data.StabilityStudies.relatedObjects;
